...
for seed in seed_list:
    ...
    n = 0
    for in_diff_prob in prob_list:
        fr.write(str(i))
        fr.write("\t")
    
        fr.write(str(in_diff_prob))
        fr.write("\t")
        experiment = 0
        vd  = Infection_Model(nodes, G,in_diff_prob , change_idea_prob , (m + 1)*nodes, runs, experiment)
        pn_diff[n].append(vd)
        fr.write(str(vd))
        fr.write("\t")
        experiment = 1
        vs  = Infection_Model(nodes, G, in_diff_prob, change_idea_prob , (m + 1)*nodes, runs, experiment)
        pn_sim[n].append(vs)
        fr.write(str(vs))
        fr.write("\n")
        n = n + 1
